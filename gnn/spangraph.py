...
class GNNLayer(nn.Module):
    ...
    def forward(self, nf, ef):
        ...
        global g
        g = self.g
        self.g.update_all(gnn_msg, gnn_reduce)
...
class SpanGraph(nn.Module):
    def __init__(self, g,  input_node_feat_size, input_edge_feat_size, n_hidden, n_classes,
                 n_layers,  activation, dropout):
        super(SpanGraph, self).__init__()
        self.layers = nn.ModuleList()
        self.layers.append(GNNLayer(g, input_node_feat_size, input_edge_feat_size, n_hidden, activation, dropout))
        for i in range(n_layers - 1):
            self.layers.append(GNNLayer(g, n_hidden, n_hidden, n_hidden, activation, dropout))
        self.layers.append(GNNLayer(g, n_hidden, n_hidden, n_classes, None, dropout))
